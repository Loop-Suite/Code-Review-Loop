...
/// Best-effort bounded join — `std::thread::JoinHandle` has no native join-with-timeout, so this
/// spawns a small relay thread that blocks on the real join and forwards the result through a
/// channel a receive-timeout can be applied to. If the target thread doesn't finish within
/// `timeout`, this just gives up (the relay thread, and whatever it was waiting on, may still be
/// running afterward) — a small, bounded cost, unlike blocking the caller indefinitely.
fn join_with_timeout<T: Send + 'static>(handle: std::thread::JoinHandle<T>, timeout: Duration) {
...
fn wait_with_timeout(
    ...
    let status = loop {
        ...
        if start.elapsed() >= timeout {
            let _ = child.kill();
            let _ = child.wait();
            // `child.wait()` above only guarantees *this* process has been reaped, not that the
            // reader threads' blocking `read_to_end` calls have seen EOF yet — a grandchild that
            // inherited the pipe's write end (e.g. a shell that forked to run its command rather
            // than exec'ing into it directly) can keep holding it open long after `child` itself
            // is gone, for as long as that grandchild happens to keep running. An *unbounded*
            // join here would defeat the entire point of this function (a real regression this
            // exact scenario caused once already — see the reader-thread-leak fix this replaced).
            // A short, bounded best-effort join reaps the common case (pipe closes promptly)
            // without risking turning a "timeout after Ns" into "hang forever."
            const READER_DRAIN_BUDGET: Duration = Duration::from_secs(1);
            join_with_timeout(stdout_handle, READER_DRAIN_BUDGET);
            join_with_timeout(stderr_handle, READER_DRAIN_BUDGET);
            return Err(anyhow!(
