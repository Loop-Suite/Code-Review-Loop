...
fn prioritize_and_cap_diff(diff: &str) -> (String, Vec<String>) {
    ...
    for (_, path, text) in indexed {
        if total + text.len() > DIFF_HARD_CAP_CHARS && !kept.is_empty() {
            // A block whose path this parser couldn't resolve (e.g. a `--no-prefix` diff, or a
            // quoted path — see `split_into_file_blocks`'s doc comment) used to vanish here with
            // no trace at all: `path` being `None` meant nothing was pushed to `dropped`, so
            // both the `[NOTE: ...]` text below and the `dropped_files` this function returns
            // (surfaced to the caller in `normalize`) stayed silent about it — directly
            // contradicting this function's own contract of reporting what got dropped instead
            // of silently truncating. A placeholder keeps the drop visible even without a name.
            dropped.push(path.unwrap_or_else(|| "(unresolved path)".to_string()));
            continue;
        }
        total += text.len();
        kept.push(text);
...
mod tests {
    ...
    #[test]
    fn prioritize_and_cap_diff_still_reports_a_drop_when_the_dropped_blocks_path_is_unresolved() {
        // Real gap: a `--no-prefix` diff block (no " b/" substring, so split_into_file_blocks
        // returns path: None for it) used to vanish silently past the cap -- excluded from `out`
        // but never pushed to `dropped`, so neither the `[NOTE: ...]` text nor the caller-visible
        // dropped_files said anything happened at all.
        let real_content = "a".repeat(DIFF_HARD_CAP_CHARS / 2 + 10);
        let noprefix_content = "l".repeat(DIFF_HARD_CAP_CHARS / 2 + 10);
        let diff = format!(
            "diff --git a/src/main.rs b/src/main.rs\n+{real_content}\n\
             diff --git secrets.env secrets.env\n+{noprefix_content}\n"
        );
        let (out, dropped) = prioritize_and_cap_diff(&diff);
        assert_eq!(
            dropped.len(),
            1,
            "the drop must be reported even without a resolvable name: {dropped:?}"
        );
        assert!(out.contains("src/main.rs"));
        assert!(
            !out.contains(&noprefix_content),
            "the dropped block's content must not survive"
        );
        assert!(out.contains("[NOTE: 1 file(s) omitted"));
    }
