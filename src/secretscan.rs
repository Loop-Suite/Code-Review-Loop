...
fn find_secrets(line: &str) -> Vec<(&'static str, &str)> {
    ...
    // use) mentions both marker strings as text, not as an actual embedded key. A real
    // *multi-line* PEM block's BEGIN and END lines are never the same physical line — the base64
    // body sits between them across multiple lines — so a single line naming both with nothing
    // real between them is a reference to the markers, not a literal key.
    if line.contains("-----BEGIN") && line.contains("PRIVATE KEY-----") {
        if !line.contains("-----END") {
            found.push(("PEM private key block", line.trim()));
        } else if has_base64_body_between_pem_markers(line) {
            // Real false negative: a complete key folded onto one physical line via literal
            // `\n` escapes (e.g. `PRIVATE_KEY="-----BEGIN...\nMII...\n-----END...\n"`) — the
            // standard form a private key takes in a `.env` file or single-line JSON
            // credential. Both markers being on the same line no longer means "just processing
            // the markers as text" once there's a real base64 body between them.
            found.push(("PEM private key block", line.trim()));
        }
    }
...
/// Distinguishes a real single-line/escaped-`\n` PEM key body from a line that merely mentions
/// both marker strings as text (e.g. a regex stripping them) — looks for a contiguous run of
/// base64-alphabet characters between the markers long enough that it can't just be punctuation.
/// A literal `\n` escape (as opposed to a real newline, which `line`s never contain in the first
/// place since scanning is line-by-line) is normalized to a real one first so it splits the run
/// the same way an actual multi-line key's line breaks would.
fn has_base64_body_between_pem_markers(line: &str) -> bool {
    const MARKER: &str = "PRIVATE KEY-----";
    let Some(body_start) = line.find(MARKER).map(|i| i + MARKER.len()) else {
        return false;
    };
    let Some(end_offset) = line[body_start..].find("-----END") else {
        return false;
    };
    let body = line[body_start..body_start + end_offset].replace("\\n", "\n");
    body.split(|c: char| !(c.is_ascii_alphanumeric() || c == '+' || c == '/' || c == '='))
        .map(str::len)
        .max()
        .unwrap_or(0)
        >= 20
...
mod tests {
    ...
    #[test]
    fn scan_finds_a_pem_key_folded_onto_one_line_via_escaped_newlines() {
        // Real false negative, reproduced against a live run before this fix: this is the
        // standard form a private key takes in a .env file or a single-line JSON credential --
        // both markers on one physical line, with literal `\n` escapes between BEGIN/body/END
        // instead of real line breaks. The prior check treated any line with both markers as
        // "just processing the markers as text" and never flagged it.
        let diff = "+++ b/.env\n@@ -1 +1 @@\n\
                     +PRIVATE_KEY=\"-----BEGIN PRIVATE KEY-----\\nMIIEvQIBADANBgkqhkiG9w0BAQEFAASCBKcwggSjAgEAAoIBAQC7VJTUt9Us8cKj\\n-----END PRIVATE KEY-----\\n\"\n";
        let hits = scan(diff);
        assert!(
            hits.iter().any(|h| h.pattern == "PEM private key block"),
            "a single-line escaped-newline PEM key must be flagged: {hits:?}"
        );
    }
