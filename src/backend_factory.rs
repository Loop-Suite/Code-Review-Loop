...
/// The host segment of a `scheme://host[:port][/path]` URL, lowercased — a real URL parser isn't
/// worth a new dependency just for the loopback check below; this is deliberately narrow, not a
...
/// A plain `http://` URL to anything other than a loopback address sends the diff,
/// `CODEREVIEW_API_KEY` (if set, as a bearer token), and the model's response over the network in
/// plaintext — refused unless the caller explicitly opts in, since `--backend custom` accepts any
/// endpoint a user configures and there's no way to know in advance whether "http://" was a
/// deliberate choice (an internal network) or a typo for "https://".
fn validate_base_url_scheme(base_url: &str, allow_insecure_http: bool) -> Result<()> {
    ...
    anyhow::ensure!(
        allow_insecure_http,
        "refusing to use a plain http:// --base-url ({base_url}) to a non-loopback host — the \
         diff, CODEREVIEW_API_KEY (if set), and the model's response would all travel in \
         plaintext. Use an https:// endpoint, or pass --allow-insecure-http if this is \
         intentional (e.g. a trusted internal network)."
    );
    eprintln!(
        "Warning: --base-url ({base_url}) is plain http:// to a non-loopback host — the diff, \
         CODEREVIEW_API_KEY (if set), and the model's response are all traveling in plaintext."
    );
    Ok(())
}
...
pub(crate) fn build_llm(cli: &Cli) -> Result<(Llm, Llm)> {
    ...
    let (main_llm, cheap_llm) = match cli.backend {
        ...
        Backend::Custom => {
            ...
            validate_base_url_scheme(&base_url, cli.allow_insecure_http)?;
            let model = cli
                .model
                .clone()
...
mod tests {
    ...
    // --- validate_base_url_scheme / --allow-insecure-http ---

    #[test]
    fn url_host_extracts_the_host_ignoring_scheme_port_and_path() {
        assert_eq!(url_host("http://example.com:8080/v1/chat"), "example.com");
        assert_eq!(url_host("https://LOCALHOST/x"), "localhost");
    ...
    #[test]
    fn validate_base_url_scheme_accepts_https_regardless_of_host() {
        assert!(validate_base_url_scheme("https://api.example.com/v1", false).is_ok());
    }

    #[test]
    fn validate_base_url_scheme_accepts_plain_http_to_a_loopback_host_without_the_flag() {
        assert!(validate_base_url_scheme("http://127.0.0.1:11434/v1", false).is_ok());
        assert!(validate_base_url_scheme("http://localhost:11434/v1", false).is_ok());
    }

    #[test]
    fn validate_base_url_scheme_refuses_plain_http_to_a_non_loopback_host_by_default() {
        // Real gap this closes: --backend custom accepted any URL, including plain http:// to a
        // real network host -- the diff and CODEREVIEW_API_KEY (if set) would travel in
        // plaintext with no warning at all.
        let err = validate_base_url_scheme("http://api.example.com/v1", false)
            .expect_err("a non-loopback plain http:// URL must be refused without the flag");
        assert!(err.to_string().contains("plaintext"));
    }

    #[test]
    fn validate_base_url_scheme_allows_plain_http_to_a_non_loopback_host_with_the_flag() {
        assert!(validate_base_url_scheme("http://api.example.com/v1", true).is_ok());
    }

    #[test]
    fn validate_base_url_scheme_rejects_a_url_with_neither_http_nor_https_scheme() {
        let err = validate_base_url_scheme("ftp://example.com/v1", false)
            .expect_err("an unrecognized scheme must be rejected");
        assert!(err.to_string().contains("http"));
    }

    #[test]
    fn build_llm_refuses_a_custom_backend_with_plain_http_to_a_public_host() {
        let cli = parse(&[
            "--backend",
            "custom",
            "--base-url",
        ...
        let err = build_llm(&cli)
            .expect_err("a public http:// endpoint must be refused without --allow-insecure-http");
        assert!(err.to_string().contains("plaintext"));
    }
