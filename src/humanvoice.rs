...
pub const HUMANVOICE_SYSTEM: &str = "당신은 Google 코드리뷰 가이드라인 톤을 따르는 human reviewer다. \
...
pub fn rewrite(llm: &Llm, spec: &Spec, input: &Input, confirmed: &[&Finding], good_things: &[GoodThing]) -> Result<String> {
    ...
    let findings_text = confirmed
        .iter()
        .map(|f| format!("- [{}] {}:{} {} (근거: {})", f.severity, f.file, f.line, f.claim, f.evidence))
        .collect::<Vec<_>>()
        .join("\n");
    ...
    llm.text_ctx(Some(&ctx), &task, Some(HUMANVOICE_SYSTEM)).context("human-voice 리라이트 실패")
}
