...
/// `run_review`'s success outcome — deliberately more than just the verdict string, because
/// verdict alone can't tell a caller like `cli::FailOn` apart from a run where every lens failed
/// and there was no defect-finding coverage at all: with zero findings, `quantify::verdict()`
/// computes plain `APPROVE` regardless of *why* there are zero findings, so `completeness` is
/// what actually distinguishes "nothing wrong was found" from "nothing could be checked."
#[derive(Debug)]
pub(crate) struct RunOutcome {
    pub verdict: String,
    pub completeness: quantify::ReviewCompleteness,
}

/// Returns the run's final outcome on success — `Err` is reserved for this run itself failing
/// (bad diff, LLM/IO error), never for "the verdict was bad" or "the review was incomplete."
pub(crate) fn run_review(llm: &Llm, cheap_llm: &Llm, args: &ReviewArgs) -> Result<RunOutcome> {
    ...
    let deterministic_handles: Vec<(
        &'static str,
        std::time::Instant,
        std::thread::JoinHandle<deterministic::ToolOutcome>,
    ...
    for (id, started, handle) in deterministic_handles {
        match handle.join() {
            Ok(deterministic::ToolOutcome::Completed(v)) => {
                println!(
            ...
            // Legitimately silent: the tool isn't installed, or (for a tool gated on changed
            // files) there was nothing relevant to check. Same as the tool never being
            // configured at all — not an error, not surfaced.
            Ok(deterministic::ToolOutcome::Skipped) => {}
            // Real gap this closes: this used to collapse into the exact same silent no-op as
            // Skipped above — a tool that WAS found and WAS invoked, but failed to spawn, timed
            // out, or produced output nothing could parse, looked identical to "not installed."
            // Synthesized into deterministic_results (under a key no spec's `deterministic_checks`
            // table names, so it won't grow a phantom row in report.rs's per-check table, but
            // `quantify::deterministic_gate` scans every entry's status regardless of key) so a
            // team relying on this tool actually sees it when it goes dark, instead of it
            // silently looking the same as never having been configured.
            Ok(deterministic::ToolOutcome::Failed(reason)) => {
                eprintln!("Warning: {id} — {reason}");
                stage_errors.push(format!("{id}: {reason}"));
                merge_deterministic_results(
                    &mut inp.deterministic_results,
                    serde_json::json!({format!("{id}_execution"): {"status": "error", "evidence": reason}}),
                );
            }
            Err(_) => {
    ...
    Ok(RunOutcome {
        ...
        completeness: quant.completeness,
    })
}
...
mod e2e_tests {
    ...
    /// Every e2e test below passes this as `deterministic_results_path` instead of `None` --
    /// `None` lets `run_review` auto-detect and actually spawn a real `semgrep`/`cargo audit` on
    /// whatever machine the test happens to run on, which used to be silently harmless (any
    /// result, or lack of one, was ignored) but stopped being harmless once
    /// `deterministic::ToolOutcome::Failed` started actually affecting the verdict: CI installs
    /// the `cargo-audit` subcommand in a step that runs *after* `cargo test`, so `cargo audit
    /// --json` legitimately fails there during the test run, which the fix correctly surfaces as
    /// NEEDS_CONTEXT -- perturbing verdicts these tests assert on for reasons that have nothing
    /// to do with what they're actually testing. A real (if empty) `--deterministic-results` file
    /// disables the auto-detect path entirely, making these tests hermetic regardless of what's
    /// installed on whatever machine runs them. Content is always identical, so a shared path
    /// (rather than one per test) is safe even under parallel test execution.
    fn empty_deterministic_results_path() -> std::path::PathBuf {
        let path =
            std::env::temp_dir().join("codereview-loop-tests-empty-deterministic-results.json");
        write_file(&path, "{}");
        path
    }
...
"#,
        ...
        let outcome = run_review(
            ...
            &ReviewArgs {
                ...
                deterministic_results_path: &Some(empty_deterministic_results_path()),
                lenses_arg: &None,
                out: &out_dir,
        ...
        assert_eq!(
            ...
            quantify::ReviewCompleteness::Complete,
            "a normal successful run must report Complete, not just leave completeness unchecked"
        );
...
"#,
        ...
        // Real, reproduced gap this closes: with zero findings (every lens failed, so none
        // exist to confirm or reject), quantify::verdict() computes plain APPROVE -- the same
        // string a genuinely clean diff produces. `outcome.completeness` is what actually lets a
        // caller (main.rs's --fail-on handling) tell "nothing wrong was found" apart from
        // "nothing could be checked at all."
        assert_eq!(outcome.verdict, "APPROVE");
        assert_eq!(
            outcome.completeness,
            quantify::ReviewCompleteness::Failed,
            "a run where every lens failed must report completeness=Failed even though the \
             verdict string alone looks like a clean APPROVE"
        );
