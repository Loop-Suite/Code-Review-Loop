...
/// Distinguishes "this tool was never going to run" from "this tool was supposed to run but
/// something went wrong" — collapsing both into the same `NOT_RUN` (as a bare `Option<Value>`
/// did) meant a genuinely broken tool (installed, invoked, then timed out / failed to spawn /
/// produced unparseable output) looked identical to a tool that was never installed in the first
/// place, or simply had nothing relevant to check. `Skipped` deliberately stays silent (an
/// optional, auto-detected tool not being on a given machine is normal, not an error); `Failed`
/// is surfaced both as a real `deterministic_gate` signal (see `quantify.rs`) and a CLI warning,
/// since a tool a team is relying on silently going dark shouldn't look the same as "not
/// configured."
...
pub(crate) fn try_run(
    ...
    let Some(bin) = crate::procutil::which(tool.binary()) else {
    ...
    let Some(output) = crate::procutil::spawn_and_wait(&bin, &args_ref, timeout) else {
        return ToolOutcome::Failed(format!(
            "{} was found on PATH but failed to spawn or exceeded its {}s timeout",
            tool.binary(),
            timeout.as_secs()
        ));
    };
    match tool.parse(&output) {
        Some(v) => ToolOutcome::Completed(v),
        None => ToolOutcome::Failed(format!(
            "{} ran (exit {:?}) but its output couldn't be parsed into a usable result",
            tool.binary(),
            output.status.code()
        )),
    }
...
mod tests {
    ...
    fn try_run_bails_without_spawning_when_changed_files_are_required_but_none_exist() {
        ...
        assert_eq!(result, ToolOutcome::Skipped);
    }
    ...
    fn try_run_runs_regardless_of_changed_files_when_not_required() {
        ...
        assert_eq!(
            result,
            ToolOutcome::Completed(serde_json::json!({"x": {"status": "pass"}}))
        );
    }

    #[test]
    fn try_run_is_skipped_not_failed_when_the_binary_is_not_on_path() {
        // Skipped, not Failed -- an optional auto-detected tool simply not being installed on a
        // given machine is normal and must stay silent, not surface as an execution error.
        struct MissingBinaryTool;
        impl DeterministicTool for MissingBinaryTool {
            fn id(&self) -> &'static str {
                "missing"
            }
            fn binary(&self) -> &'static str {
                "this-binary-does-not-exist-anywhere-on-any-path-12345"
            }
            fn default_timeout(&self) -> Duration {
                Duration::from_secs(5)
            }
        ...
        let result = try_run(&MissingBinaryTool, &[], Duration::from_secs(5));
        assert_eq!(result, ToolOutcome::Skipped);
    }

    #[test]
    fn try_run_fails_distinctly_from_skipped_when_parse_returns_none() {
        // Real gap this closes: the tool was found and actually ran (it's on PATH, the gate
        // passed, spawn succeeded) -- unlike the "not installed"/"nothing to check" cases above,
        // this is a tool that was supposed to work and didn't, which used to collapse into the
        // exact same `None` as those legitimate skips.
        let tool = FakeTool {
            requires_changed_files: false,
            parsed: None,
        };
        ...
        match result {
            ToolOutcome::Failed(reason) => {
                assert!(reason.contains("output couldn't be parsed"), "{reason}")
            }
