...
pub(crate) struct Cli {
    ...
    /// `--backend custom`'s `--base-url` must be `https://` (or a loopback host — `127.0.0.1`,
    /// `localhost`, `::1` — for a local dev endpoint like Ollama) unless this is set. A plain
    /// `http://` URL to anything else sends the diff, `CODEREVIEW_API_KEY` (if set), and the
    /// model's response over the network in plaintext.
    #[arg(long, global = true)]
