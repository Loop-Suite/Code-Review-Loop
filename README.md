...
The implementation is a 12-step pipeline (`src/pipeline/review.rs`'s own `// Step N` comments number
each one) condensed into 11 rows below — steps 1-2 (input normalization, convention injection) and
steps 8-9 (discourse's two sub-phases) each share one row, since they're handled together in
practice. Most important modules:
...
- Post the report as a PR comment or check-run **annotation**, not a required status check that
  ...
  (`--fail-on never`) keeps today's non-blocking behavior unchanged. Any other `--fail-on` value
  also exits non-zero if every selected lens failed (a provider outage, a bad `--base-url`, etc.)
  — zero findings from zero coverage computes the same `APPROVE` a genuinely clean diff would, so
  this is checked independently of the verdict string itself; pass `--fail-on never` if you want
  a completely failed run to still exit 0.
